        ...
        <li key={project.slug} className='group relative'>
          <Link href={`/projects/${project.slug}`}>
            {project.image && (
              <div className='bg-muted h-72 w-full overflow-hidden sm:h-60'>
                <Image
                  ...
                  alt={project.title || ''}
                  fill={true}
                  className='size-auto rounded-lg object-cover object-center transition-transform duration-500 group-hover:scale-105'
                />
              </div>
            )}
            ...
            <div className='absolute inset-x-0 bottom-0 translate-y-2 px-6 py-5 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100'>
              ...
              <p className='text-muted-foreground text-xs font-light'>{formatDate(project.publishedAt ?? '')}</p>
            </div>
          </Link>
        </li>
      ))}
